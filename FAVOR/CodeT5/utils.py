...
def load_data(args,filename,pool,tokenizer,stage):

    examples = read_examples(filename,data_num=None,task=args.task)
    index = torch.tensor([e.idx for e in examples], dtype=torch.long)
    ...
    data = TensorDataset(all_source_ids, all_target_ids,index)
...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if os.path.exists(cache_fn):
        ...
        all_source_ids = torch.tensor([f.source_ids for f in features], dtype=torch.long)
        all_labels = torch.tensor([f.label for f in features], dtype=torch.long)
        all_indices = torch.tensor([f.idx for f in features], dtype=torch.long)
        data = TensorDataset(all_source_ids, all_labels, all_indices)
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        # data_dir = '{}/{}'.format(data_root, task)
        data_dir = '/home/<ANON>/code/ddfa/code_gnn/LineVul-code_gnn/data/codet5_MSR'
        train_fn = '{}/train.jsonl'.format(data_dir)
        dev_fn = '{}/valid.jsonl'.format(data_dir)
        test_fn = '{}/test.jsonl'.format(data_dir)
    elif task == 'repair':
        data_dir = '/home/l1/qingao/DeepDFA/DDFA/storage/external'
        train_fn = f'{data_dir}/train_data.json'
        dev_fn = f'{data_dir}/val_data.json'
        test_fn = f'{data_dir}/test_data.json'
    if split == 'train':
        return train_fn
    elif split == 'dev':
        return dev_fn
