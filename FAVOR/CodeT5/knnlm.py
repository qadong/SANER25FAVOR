...
class KNNWrapper(object):
    def __init__(self, dstore_size, dstore_dir, dimension, 
        ...
        # print(knn_sim_func)
        self.dist_func = KNNWrapper.l2 # l2 or dot product function
    ...
    def setup_faiss(self):
        ...
        if not self.no_load_keys:
            self.keys = np.memmap(f'{keys_vals_prefix}_keys.npy', dtype=np.float32, mode='r',
                                  shape=(self.dstore_size, self.dimension))
        self.vals = np.memmap(f'{keys_vals_prefix}_vals.npy', dtype=np.int32, mode='r',
                              shape=(self.dstore_size, 1))
        
        self.source = np.memmap(f'{keys_vals_prefix}_source.npy', dtype=np.int32, mode='r',
                              shape=(self.dstore_size, 512))
        ...
        if self.move_dstore_to_mem:
            ...
            if not self.no_load_keys:
                ...
                self.keys_from_memmap = np.memmap(f'{keys_vals_prefix}_keys.npy', 
                    dtype=np.float32, mode='r', shape=(self.dstore_size, self.dimension))
            ...
            del self.source
            source_from_memap = np.memmap(f'{keys_vals_prefix}_source.npy',dtype=np.int32, mode='r', shape=(self.dstore_size, 512))
            self.source = torch.from_numpy(source_from_memap[:]).long().to(self.device)
            del source_from_memap
    ...
    def pre_forward_hook(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        ...
        #     print("Warning: input_ids is None.")
        self.source_now = input_ids
    ...
    def knns_to_log_prob(self, knns, neg_dists):
        ...
        knn_indices_np = knns.cpu().numpy() if knns.is_cuda else knns.numpy()
        vals_at_knns = self.vals[knn_indices_np].squeeze(-1) # (nonpad batch * time, k)
        vals_at_knns_tensor = torch.tensor(vals_at_knns, dtype=torch.int64, device=self.device)
        knn_log_probs = torch.full(size=(vals_at_knns.shape[:-1] + (self.vocab_size,)), fill_value=0.0).to(self.device) \
            .scatter_add(dim=-1, index=vals_at_knns_tensor, src=probs).log() # (nonpad_batch * time, vocab)
        knn_log_probs = torch.nan_to_num(knn_log_probs, nan=None, neginf=-10000.0)
        # print('input_source',self.tokenizer.decode(self.source_now,skip_special_tokens=True))
        # for index,dist in zip(knn_indices_np[0][:],neg_dists):
        #     print('_'*20)
        #     print(self.tokenizer.decode(self.source[index],skip_special_tokens=True),dist)
...
class KNNSaver(object):
    ...
    def __init__(self, dstore_size, dstore_dir, dimension, tokenizer,knn_keytype=None):
        self.dstore_size = dstore_size
        self.dstore_dir = dstore_dir
        self.dimension = dimension
        self.knn_keytype = KEY_TYPE.last_ffn_input if knn_keytype is None else knn_keytype
    ...
    def break_into(self, model):
        ...
        source_filename = f'{keys_vals_prefix}_source.npy'
        if os.path.exists(keys_filename) and os.path.exists(vals_filename) and os.path.exists(source_filename):
            mode = 'r'
        else:
            mode = 'w+'
            Path(keys_filename).parent.mkdir(parents=True, exist_ok=True)
        
        self.dstore_keys = np.memmap(keys_filename, dtype=np.float32, mode=mode, shape=(self.dstore_size, self.dimension))
        self.dstore_vals = np.memmap(vals_filename, dtype=np.int32, mode=mode, shape=(self.dstore_size, 1))
        self.dstore_source = np.memmap(source_filename, dtype=np.int32, mode=mode, shape=(self.dstore_size, 512) )
    def pre_forward_hook(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        if labels is None:
    ...
    def post_forward_hook(self, module, input, output):
        ...
        # print(keys.size(), values.size())
        # if shape[0] == args.tokens_per_sample:
        # expanded_source_ids = self.source_ids.expand(batch_time_size, -1)
        ...
        try:
            self.dstore_keys[self.dstore_idx:(batch_time_size + self.dstore_idx)] = keys.cpu().numpy().astype(np.float32)
            self.dstore_vals[self.dstore_idx:(batch_time_size + self.dstore_idx)] = values.unsqueeze(-1).cpu().numpy().astype(np.int32)
            # self.dstore_source[self.dstore_idx:(batch_time_size + self.dstore_idx)] = expanded_source_ids.cpu().numpy()
