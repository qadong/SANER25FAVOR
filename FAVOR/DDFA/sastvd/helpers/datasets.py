...
import json
import traceback
import DDFA.sastvd.helpers.git as svdg
...
def combined(cache=True,sample=False):
    """
    read combined dataset from csv
    """

    savefile = (svd.get_dir()/"combined_dataset")
...
def ds(dsname, cache=True, sample=False):
    if dsname == "bigvul":
        return bigvul(cache=False, sample=sample)
    elif dsname == "devign":
...
def ds_filter(
    ...
    print("Filter dataset based on various considerations for training:")
    # for key, value in locals().items():
    #     if key != 'self':  # 不打印 self
    #         print(f"{key}: {value}")
    ...
    if check_file:
        ...
        print(svd.processed_dir() / dsname / "before/*nodes*")
...
def get_linevul_splits():
    ...
    splits_df = pd.read_csv(svd.external_dir() / "data_split.csv", index_col=0)
